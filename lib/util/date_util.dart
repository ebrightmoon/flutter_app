...
class XFSDateUtil {
  ...
  static String getDateStrByTimeStr(
      String dateStr, {
        XFSDateFormat format = XFSDateFormat.NORMAL,
        String dateSeparate,
        String timeSeparate,
        bool isUtc,
      }) {
  ...
  static String formatZHDateTime(
      String time, XFSDateFormat format, String timeSeparate) {
    time = convertToZHDateTimeString(time, timeSeparate);
    switch (format) {
      case XFSDateFormat.ZH_NORMAL: //yyyy年MM月dd日 HH时mm分ss秒
        time = time.substring(
            0,
            "yyyy年MM月dd日 HH时mm分ss秒".length -
                (timeSeparate == null || timeSeparate.isEmpty ? 0 : 1));
        break;
      case XFSDateFormat.ZH_YEAR_MONTH_DAY_HOUR_MINUTE: //yyyy年MM月dd日 HH时mm分
        time = time.substring(
            0,
            "yyyy年MM月dd日 HH时mm分".length -
                (timeSeparate == null || timeSeparate.isEmpty ? 0 : 1));
        break;
      case XFSDateFormat.ZH_YEAR_MONTH_DAY: //yyyy年MM月dd日
        time = time.substring(0, "yyyy年MM月dd日".length);
        break;
      case XFSDateFormat.ZH_YEAR_MONTH: //yyyy年MM月
        time = time.substring(0, "yyyy年MM月".length);
        break;
      case XFSDateFormat.ZH_MONTH_DAY: //MM月dd日
        time = time.substring("yyyy年".length, "yyyy年MM月dd日".length);
        break;
      case XFSDateFormat.ZH_MONTH_DAY_HOUR_MINUTE: //MM月dd日 HH时mm分
        time = time.substring(
            "yyyy年".length,
            "yyyy年MM月dd日 HH时mm分".length -
                (timeSeparate == null || timeSeparate.isEmpty ? 0 : 1));
        break;
      case XFSDateFormat.ZH_HOUR_MINUTE_SECOND: //HH时mm分ss秒
        time = time.substring(
            "yyyy年MM月dd日 ".length,
            "yyyy年MM月dd日 HH时mm分ss秒".length -
                (timeSeparate == null || timeSeparate.isEmpty ? 0 : 1));
        break;
      case XFSDateFormat.ZH_HOUR_MINUTE: //HH时mm分
        time = time.substring(
            "yyyy年MM月dd日 ".length,
            "yyyy年MM月dd日 HH时mm分".length -
                (timeSeparate == null || timeSeparate.isEmpty ? 0 : 1));
  ...
  /// dateSeparate    date separate.
  /// timeSeparate    time separate.
  static String formatDateTime(String time, XFSDateFormat format,
      String dateSeparate, String timeSeparate) {
    switch (format) {
      case XFSDateFormat.NORMAL: //yyyy-MM-dd HH:mm:ss
        time = time.substring(0, "yyyy-MM-dd HH:mm:ss".length);
        break;
      case XFSDateFormat.YEAR_MONTH_DAY_HOUR_MINUTE: //yyyy-MM-dd HH:mm
        time = time.substring(0, "yyyy-MM-dd HH:mm".length);
        break;
      case XFSDateFormat.YEAR_MONTH_DAY: //yyyy-MM-dd
        time = time.substring(0, "yyyy-MM-dd".length);
        break;
      case XFSDateFormat.YEAR_MONTH: //yyyy-MM
        time = time.substring(0, "yyyy-MM".length);
        break;
      case XFSDateFormat.MONTH_DAY: //MM-dd
        time = time.substring("yyyy-".length, "yyyy-MM-dd".length);
        break;
      case XFSDateFormat.MONTH_DAY_HOUR_MINUTE: //MM-dd HH:mm
        time = time.substring("yyyy-".length, "yyyy-MM-dd HH:mm".length);
        break;
      case XFSDateFormat.HOUR_MINUTE_SECOND: //HH:mm:ss
        time =
            time.substring("yyyy-MM-dd ".length, "yyyy-MM-dd HH:mm:ss".length);
        break;
      case XFSDateFormat.HOUR_MINUTE: //HH:mm
  ...
  static bool isZHFormat(XFSDateFormat format) {
    return format == XFSDateFormat.ZH_DEFAULT ||
        format == XFSDateFormat.ZH_NORMAL ||
        format == XFSDateFormat.ZH_YEAR_MONTH_DAY_HOUR_MINUTE ||
        format == XFSDateFormat.ZH_YEAR_MONTH_DAY ||
        format == XFSDateFormat.ZH_YEAR_MONTH ||
        format == XFSDateFormat.ZH_MONTH_DAY ||
        format == XFSDateFormat.ZH_MONTH_DAY_HOUR_MINUTE ||
        format == XFSDateFormat.ZH_HOUR_MINUTE_SECOND ||
        format == XFSDateFormat.ZH_HOUR_MINUTE;
  }
  ...
  static String formatDate(DateTime dateTime, {bool isUtc, String format}) {
    ...
    format = format ?? XFSDataFormats.full;
